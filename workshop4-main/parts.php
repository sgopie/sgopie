...
<?php
    ...
    $partCriteria = new PartCriteria('part', 'id');

    if (isset($_POST['search'])) {
        $name = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        $description = filter_input(INPUT_POST, 'description', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        if(isset($name)){
            $partCriteria->setName($name);
        }
        if(isset($description)){
            $partCriteria->setDescription($description);
        }
    }
    //sorteren op id
    if (isset($_POST['order-part-id'])) {
        $ordering = filter_input(INPUT_POST, 'ordering', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        $partCriteria->setOrdering($ordering === 'ASC' ? 'DESC' : 'ASC');
        $partCriteria->setOrderField('id');
    }
    //sorteren op naam
    if (isset($_POST['order-part-name'])) {
        $ordering = filter_input(INPUT_POST, 'ordering', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        $partCriteria->setOrdering($ordering === 'ASC' ? 'DESC' : 'ASC');
        $partCriteria->setOrdering('name');
    }

    $partCriteria->setFromJoins('part');
    $partCriteria->setVendorId($vendorId);
    $parts = findByCriteria($partCriteria);
...
<main>
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-13">
                <h2>zoeken</h2>
                <form method="post" action="parts.php?id=<?=$partCriteria->getVendorId()?>">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="inputName" class="form-label">naam</label>
                            <input type="text" name="name" class="form-control" id="inputName" value="<?=$partCriteria->getName() ?? ""?>">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="inputDescription" class="form-label">beschrijving</label>
                            <input type="text" name="description" class="form-control" id="inputdescription" value="<?=$partCriteria->getDescription() ?? ""?>">
                        </div>
                        <div class="col-md-6 mb-3">
                ...
                <form method="post" action="parts.php?id=<?=$partCriteria->getVendorId()?>">
                    <div class="row">
                        <div class="col-md-2 mb-3">
                            <input type="hidden" name="ordering" value="<?=$partCriteria->getOrdering()?>">
                            <button class="btn btn-primary" type="submit" name="order-part-id">Sort by id<i class="bi bi-arrow-down-up"></i></button>
                        </div>
                        <div class="col-md-2 mb-3">
                            <input type="hidden" name="ordering" value="<?= $partCriteria->getOrdering()?>">
                            <button class="btn btn-primary" type="submit" name="ordering-part-name">Sort by name <i class="bi bi-arrow-down-up"></i></button>
                        </div>
                    </div>
                </form>
            </div>
